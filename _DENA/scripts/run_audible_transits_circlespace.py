...
class AudibleTransits:
    ...
    def export_results(self, export_garbage=False):
        ...
        '''
        identifier = self.unit+self.site+str(self.year)+'_'+str(self.gain)+'_circle_'+self.database_type
...
if __name__ == '__main__':
    ...
    active_space = listener.active.copy()
    active_area = active_space.iloc[0].area
    equivalent_radius = math.sqrt(active_area/math.pi)
    active_circle = listener.mic.geometry.iloc[0].buffer(equivalent_radius)
    active_circle_gdf = gpd.GeoSeries(active_circle, crs=listener.utm_zone)
    listener.active = active_circle_gdf
