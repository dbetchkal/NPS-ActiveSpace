...
- **Path length:** AAM 3.0.0 stores ``ROTOR_NOISE`` / ``NCfiles`` paths in a Fortran ``FILENAME`` buffer (140 chars). Long site paths plus descriptive run folder names exceed that on network shares. Runs use hashed ``Output_Data/aam/runs/x{hash12}/`` dirs; full job names are mapped in ``active_space.log`` (``[aam-run-dir]``).
