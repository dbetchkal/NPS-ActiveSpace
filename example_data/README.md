...
### What's in git vs regenerated

Committed under each site dir: study area shapefile, DEM `.tif`, `.sit`, and ground-truthing annotation `.geojson` files.
Regenerated by pipeline runs (gitignored under `example_data/site_projects/`): `Output_Data/`, `fits.csv`, elevation masks, diagnostic plots, and NMSim scratch files.

On Mac/Linux, NMSim active-space generation runs in Docker + Wine (`-e container`); see [docker/README.md](../docker/README.md).
Use `-e DENA_example` / `-e GLBA_example` for native ground-truthing and viz.
...
| `site_projects/` | Per-site workspaces (`GLBALSTL/`, `DENATRLA/`) — DEM, study area, `.sit`, annotation `.geojson` |
...
### DENATRLA 2025-06-23 slice (~30 MB in git; shared archive + ADS-B)

| Path | Size (approx) | Notes |
|------|---------------|-------|
| `nvspl_archive/2025 DENATRLA Triple Lakes/01 DATA/NVSPL/` | 23 MB | 24 hourly `NVSPL_DENATRLA_2025_06_23_*.txt` |
| `site_projects/DENATRLA/` | 4.2 MB | DEM, study area, `.sit`, `DENATRLA2025_saved_annotations.geojson`, clock drift CSVs |
